...
export async function POST(request: Request) {
  try {
    ...
    const result = await askArlo({ workspace: "ResolveX", context, messages: input.messages });
    return NextResponse.json({ message: result.message, source: "ResolveX approved product guide" });
  } catch (error) {
    const detail = error instanceof Error ? error.message : "Invalid request";
    return NextResponse.json({ message: "I could not complete that answer. I can still keep your question ready for a person.", source: "Human handoff available", detail }, { status: 200 });
  }
}
