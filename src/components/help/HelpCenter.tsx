        ...
        <div className="relative z-10 flex h-16 items-center gap-3 rounded-[7px] border border-black/12 bg-[#f7f7f4] px-5 shadow-[0_18px_55px_rgba(25,30,42,.08)]">
          ...
          {query && (
            <button
              type="button"
              onClick={() => setQuery("")}
              className="grid size-9 place-items-center rounded-[5px] text-[#6f7279] transition hover:bg-black/5 hover:text-black"
        ...
        {normalized ? (
          <div className="mt-10" aria-live="polite">
            <div className="flex items-end justify-between gap-4 border-b border-black/10 pb-4">
              ...
              <span className="text-xs text-[#777a81]">
                for “{query.trim()}”
              </span>
            </div>
            {results.length ? (
                ...
                {results.map((article) => {
                  ...
                  return (
                    <Link
                      ...
                      className="group grid gap-2 py-6 sm:grid-cols-[1fr_auto] sm:items-center"
                    >
                      <span>
                        <span className="text-[10px] font-bold uppercase tracking-[.12em] text-[#ff5c35]">
                          {category?.title}
                        </span>
                        <span className="mt-2 block text-xl font-semibold tracking-[-.02em] group-hover:text-[#ff5c35]">
                          {article.title}
                        </span>
                        <span className="mt-2 block max-w-3xl text-sm leading-relaxed text-[#6f7279]">
                          {article.summary}
                        </span>
                      </span>
              ...
              <div className="border-b border-black/10 py-16 text-center">
                ...
                <p className="mt-2 text-sm text-[#74777e]">
                  Try “widget”, “billing”, “Arlo”, “roles”, or “routing”.
                </p>
              </div>
            )}
          </div>
              ...
              {helpCategories.map((category) => {
                ...
                return (
                  <a
                    key={category.slug}
                    href={`#${category.slug}`}
                    className="group rounded-[7px] border border-black/10 p-6 transition duration-300 hover:-translate-y-1 hover:border-black/20 hover:shadow-[0_22px_55px_rgba(25,30,42,.1)]"
                  >
                    <Icon size={21} className="text-[#ff5c35]" />
                    <h2 className="mt-5 text-xl font-semibold">
                      {category.title}
                    </h2>
                    <p className="mt-2 text-sm leading-relaxed text-[#70737a]">
                      {category.description}
                    </p>
                    <span className="mt-6 flex items-center gap-2 text-xs font-semibold">
                      Browse {count} articles{" "}
                      <ArrowRight
                        size={14}
                        className="transition group-hover:translate-x-1"
                      />
              ...
              {helpCategories.map((category) => (
                ...
                >
                  <div className="grid gap-4 border-b border-black/12 pb-5 md:grid-cols-[.42fr_.58fr] md:items-end">
                    ...
                    <p className="text-sm leading-relaxed text-[#73767d]">
                      {category.description}
                    </p>
                  </div>
                  <div className="divide-y divide-black/9">
                    {helpArticles
                      ...
                      .map((article) => (
                        ...
                        >
                          <span>
                            <span className="block text-lg font-semibold tracking-[-.02em] group-hover:text-[#ff5c35]">
                              {article.title}
                            </span>
                            <span className="mt-1.5 block max-w-3xl text-sm leading-relaxed text-[#73767d]">
                              {article.summary}
                            </span>
                          </span>
                          <span className="flex items-center gap-2 text-xs font-semibold text-[#565960]">
                            {article.readTime}
                            <ArrowRight
                              size={14}
                              className="transition group-hover:translate-x-1"
